document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Ensure the remove buttons work via event delegation
      // (attached outside so it's set only once)
    } catch (error) {
  ...
  activitiesList.addEventListener("click", async (e) => {
    // Robustly find the remove button even if the click target is a text node
    let el = e.target;
    let btn = null;
    while (el && el !== activitiesList) {
      if (el.classList && el.classList.contains && el.classList.contains("participant-remove")) {
        btn = el;
        break;
      }
      el = el.parentNode;
